...
export async function loadMathFunction(
  ...
  await client.functionLoad(MATH_FUNCTION.code, { REPLACE: true });
}
...
describe("Client", () => {
  describe("parseURL", () => {
    ...
    it("valkey://localhost", () => {
      assert.deepEqual(ValkeyClient.parseURL("valkey://localhost"), {
        socket: {
          host: "localhost",
        },
      });
  ...
  describe("connect", () => {
    testUtils.testWithClient(
      "connect should return the client instance",
      async (client) => {
        try {
    ...
    testUtils.testWithClient(
      "should set default lib name and version",
      async (client) => {
    ...
    testUtils.testWithClient(
      "disable sending lib name and version",
      async (client) => {
        const clientInfo = await client.clientInfo();
  ...
  describe("authentication", () => {
    ...
    testUtils.testWithClient(
      "should execute AUTH before SELECT",
      async (client) => {
        assert.equal((await client.clientInfo()).db, 2);
      },
      {
        ...GLOBAL.SERVERS.PASSWORD,
        clientOptions: {
  ...
  testUtils.testWithClient(
    "should set connection name",
    async (client) => {
      assert.equal(await client.clientGetName(), "name");
    },
    {
      ...GLOBAL.SERVERS.OPEN,
      clientOptions: {
  ...
  describe("legacyMode", () => {
    testUtils.testWithClient(
      "client.sendCommand should call the callback",
      async (client) => {
        assert.equal(
    ...
    testUtils.testWithClient(
      "client.sendCommand should work without callback",
      async (client) => {
    ...
    testUtils.testWithClient(
      "client.sendCommand should reply with error",
      async (client) => {
    ...
    testUtils.testWithClient(
      "client.hGetAll should reply with error",
      async (client) => {
        await assert.rejects(promisify(client.hGetAll).call(client));
      },
      {
        ...GLOBAL.SERVERS.OPEN,
        clientOptions: {
    ...
    testUtils.testWithClient(
      "client.v4.sendCommand should return a promise",
      async (client) => {
        assert.equal(await client.v4.sendCommand(["PING"]), "PONG");
      },
      {
        ...GLOBAL.SERVERS.OPEN,
        clientOptions: {
    ...
    testUtils.testWithClient(
      "client.v4.{command} should return a promise",
      async (client) => {
        assert.equal(await client.v4.ping(), "PONG");
      },
      {
        ...GLOBAL.SERVERS.OPEN,
        clientOptions: {
    ...
    testUtils.testWithClient(
      "client.{command} should accept vardict arguments",
      async (client) => {
        assert.equal(await promisify(client.set).call(client, "a", "b"), "OK");
      },
      {
        ...GLOBAL.SERVERS.OPEN,
        clientOptions: {
    ...
    testUtils.testWithClient(
      "client.{command} should accept arguments array",
      async (client) => {
        assert.equal(
    ...
    testUtils.testWithClient(
      "client.{command} should accept mix of arrays and arguments",
      async (client) => {
        assert.equal(
    ...
    testUtils.testWithClient(
      "client.hGetAll should return object",
      async (client) => {
    ...
    >(
      ...
      return new Promise((resolve, reject) => {
        (multi as any).exec(
          (err: Error | undefined, replies: Array<ValkeyCommandRawReply>) => {
    ...
    testUtils.testWithClient(
      "client.multi.ping.exec should call the callback",
      async (client) => {
        assert.deepEqual(await multiExecAsync(client.multi().ping()), ["PONG"]);
      },
      {
        ...GLOBAL.SERVERS.OPEN,
        clientOptions: {
    ...
    testUtils.testWithClient(
      "client.multi.ping.v4.ping.v4.exec should return a promise",
      async (client) => {
        assert.deepEqual(await client.multi().ping().v4.ping().v4.exec(), [
          "PONG",
          "PONG",
        ]);
      },
    ...
    testUtils.testWithClient(
      "client.{script} should return a promise",
      async (client) => {
    ...
    testUtils.testWithClient(
      "client.multi.{command}.exec should flatten array arguments",
      async (client) => {
        assert.deepEqual(await client.multi().sAdd("a", ["b", "c"]).v4.exec(), [
          2,
        ]);
      },
      {
    ...
    testUtils.testWithClient(
      "client.multi.hGetAll should return object",
      async (client) => {
        assert.deepEqual(
          await multiExecAsync(
            client.multi().hSet("key", "field", "value").hGetAll("key")
          ),
          [
            1,
            Object.create(null, {
  ...
  describe("events", () => {
    testUtils.testWithClient(
      ...
      async (client) => {
        ...
        await Promise.all([once(client, "end"), client.disconnect()]);
      },
      {
        ...GLOBAL.SERVERS.OPEN,
        disableClientSetup: true,
  ...
  describe("sendCommand", () => {
    ...
    testUtils.testWithClient(
      "undefined and null should not break the client",
      async (client) => {
        await assert.rejects(
  ...
  describe("multi", () => {
    testUtils.testWithClient(
      ...
      async (client) => {
        assert.deepEqual(
          await client.multi().ping().set("key", "value").get("key").exec(),
          ["PONG", "OK", "value"]
        );
      },
      GLOBAL.SERVERS.OPEN
    ...
    testUtils.testWithClient(
      "should reject the whole chain on error",
      (client) => {
        return assert.rejects(
          client.multi().ping().addCommand(["INVALID COMMAND"]).ping().exec()
        );
      },
      GLOBAL.SERVERS.OPEN
    );
    ...
    testUtils.testWithClient(
      "should reject the whole chain upon client disconnect",
      async (client) => {
        await client.disconnect();

        return assert.rejects(
          client.multi().ping().set("key", "value").get("key").exec(),
          ClientClosedError
        );
      },
      GLOBAL.SERVERS.OPEN
    ...
    testUtils.testWithClient(
      "with script",
      async (client) => {
        assert.deepEqual(await client.multi().square(2).exec(), [4]);
      },
      {
        ...GLOBAL.SERVERS.OPEN,
        clientOptions: {
    ...
    describe("execAsPipeline", () => {
      testUtils.testWithClient(
        "exec(true)",
        async (client) => {
          assert.deepEqual(await client.multi().ping().exec(true), ["PONG"]);
        },
        GLOBAL.SERVERS.OPEN
      );

      testUtils.testWithClient(
        "empty execAsPipeline",
        async (client) => {
          assert.deepEqual(await client.multi().execAsPipeline(), []);
        },
        GLOBAL.SERVERS.OPEN
      );
    });
    ...
    testUtils.testWithClient(
      "should remember selected db",
      async (client) => {
        await client.multi().select(1).exec();
        await killClient(client);
        assert.equal((await client.clientInfo()).db, 1);
      },
      {
        ...GLOBAL.SERVERS.OPEN,
    ...
    testUtils.testWithClient(
      "should handle error replies (#2665)",
      async (client) => {
        await assert.rejects(
          client.multi().set("key", "value").hGetAll("key").exec(),
          (err) => {
  ...
  testUtils.testWithClient(
    "modules",
    async (client) => {
      assert.equal(await client.module.echo("message"), "message");
    },
    {
      ...GLOBAL.SERVERS.OPEN,
      clientOptions: {
  ...
  testUtils.testWithClient(
    ...
    async (client) => {
      ...
      assert.equal(await client.math.square(2), 4);
    },
    {
      ...GLOBAL.SERVERS.OPEN,
  ...
  describe("isolationPool", () => {
    testUtils.testWithClient(
      ...
      async (client) => {
        const id = await client.clientId(),
          isolatedId = await client.executeIsolated((isolatedClient) =>
            isolatedClient.clientId()
          );
    ...
    testUtils.testWithClient(
      "should be able to use pool even before connect",
      async (client) => {
    ...
    testUtils.testWithClient(
      "should work after reconnect (#2406)",
      async (client) => {
        await client.disconnect();
        await client.connect();
        await client.executeIsolated(() => Promise.resolve());
    ...
    testUtils.testWithClient(
      "should throw ClientClosedError after disconnect",
      async (client) => {
        await client.connect();
        await client.disconnect();
        await assert.rejects(
  ...
  testUtils.testWithClient(
    "should reconnect when socket disconnects",
    async (client) => {
      await killClient(client);
  ...
  testUtils.testWithClient(
    'should propagated errors from "isolated" clients',
    (client) => {
      client.on("error", () => {
        // ignore errors
      });
  ...
  testUtils.testWithClient(
    ...
    async (client) => {
      ...
      assert.deepEqual(
        ...
        members
          .map<MemberTuple>((member) => [member.value, member.score])
          .sort(sort)
      );
    },
    GLOBAL.SERVERS.OPEN
  ...
  describe("PubSub", () => {
    testUtils.testWithClient(
      "should be able to publish and subscribe to messages",
      async (publisher) => {
        ...
        try {
          ...
          assert.ok(
            channelListener1.calledOnceWithExactly(
              Buffer.from("message"),
              Buffer.from("channel")
            )
          );
          assert.ok(
            channelListener2.calledOnceWithExactly("message", "channel")
          );
          assert.ok(
            patternListener.calledOnceWithExactly("message", "channel")
          );
          ...
          assert.ok(
            channelListener2.secondCall.calledWithExactly("message", "channel")
          );
          assert.ok(patternListener.calledTwice);
          assert.ok(
            patternListener.secondCall.calledWithExactly("message", "channel")
          );
          ...
          assert.ok(
            patternListener.thirdCall.calledWithExactly("message", "channel")
          );
    ...
    testUtils.testWithClient(
      "should not fail when message arrives right after subscribe",
      async (publisher) => {
        const subscriber = publisher.duplicate();
    ...
    testUtils.testWithClient(
      "should be able to quit in PubSub mode",
      async (client) => {
  ...
  testUtils.testWithClient(
    "should be able to connect after disconnect (see #1801)",
    async (client) => {
      await client.disconnect();
      await client.connect();
    },
  ...
  testUtils.testWithClient(
    "should be able to use ref and unref",
    (client) => {
      client.unref();
      client.ref();
    },
  ...
  testUtils.testWithClient(
    "pingInterval",
    async (client) => {
      assert.deepEqual(await once(client, "ping-interval"), ["PONG"]);
    },
    {
      ...GLOBAL.SERVERS.OPEN,
      clientOptions: {
  ...
  testUtils.testWithClient(
    "should reject commands in connect phase when `disableOfflineQueue`",
    async (client) => {
