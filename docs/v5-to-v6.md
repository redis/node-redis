...
| `@redis/time-series` | `TS.MRANGE SELECTED_LABELS GROUPBY` | `{ labels: Record<string, string \| null>; samples: Array<{ timestamp: number; value: number }> } -> { labels: Record<string, string \| null>; sources: Array<string>; samples: Array<{ timestamp: number; value: number }> }` | RESP3 selected-labels grouped reply now includes `sources`. |
| `@redis/time-series` | `TS.MREVRANGE SELECTED_LABELS GROUPBY` | `{ labels: Record<string, string \| null>; samples: Array<{ timestamp: number; value: number }> } -> { labels: Record<string, string \| null>; sources: Array<string>; samples: Array<{ timestamp: number; value: number }> }` | RESP3 selected-labels grouped reverse-range replies now include `sources` alongside `{ labels, samples }`. |
