...
export class PubSub {
  ...
  unsubscribe<T extends boolean>(
    ...
    if (args.length === 1) {
      ...
      for (const channel of channelsArray) {
        PubSub.#listenersSet(listeners.get(channel)!, returnBuffers).delete(
          listener
        );
      }
      return;
  ...
  resubscribe(): Array<PubSubCommand> {
    ...
    for (const [type, listeners] of Object.entries(this.#listeners)) {
      ...
      commands.push({
        args: [COMMANDS[type as PubSubType].subscribe, ...listeners.keys()],
  ...
  #emitPubSubMessage(
    ...
    const channelString = pattern ? channel.toString() : keyString,
      messageString =
        channelString === "__redis__:invalidate"
          ? // https://github.com/redis/redis/pull/7469
            // https://github.com/redis/redis/issues/7463
            ((message === null
              ? null
              : (message as any as Array<Buffer>).map((x) =>
                  x.toString()
                )) as any)
          : message.toString();
    for (const listener of listeners.strings) {
