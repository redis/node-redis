...
describe('Graph', () => {
    testUtils.testWithClient('Graph', async client => {
        const graph = new Graph(client as any, 'a');

        await client.graph.query('a', 'CREATE (:person {name: "leibale", array: [1,2,"a"]})-[:knows]->(:person {name: "amit"})');
        console.log(JSON.stringify(
            (await graph.query('MATCH path = (p :person)-[:knows]->(:person) RETURN p, path') as any)[0]
        ));
    }, GLOBAL.SERVERS.OPEN);
