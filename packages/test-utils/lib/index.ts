import {
  ...
  ValkeyFunctions,
  ValkeyScripts,
} from "@valkey/client/lib/commands";
import ValkeyClient, {
  ValkeyClientOptions,
  ValkeyClientType,
} from "@valkey/client/lib/client";
import ValkeyCluster, {
  ValkeyClusterOptions,
  ValkeyClusterType,
} from "@valkey/client/lib/cluster";
import { ValkeySocketCommonOptions } from "@valkey/client/lib/client/socket";
import {
  ValkeyServerDockerConfig,
  spawnValkeyServer,
  spawnValkeyCluster,
} from "./dockers";
...
> extends CommonTestOptions {
  ...
  clientOptions?: Partial<
    Omit<ValkeyClientOptions<M, F, S>, "socket"> & {
      socket: ValkeySocketCommonOptions;
    }
  >;
...
export default class TestUtils {
  ...
  constructor(config: TestUtilsConfig) {
    const { string, numbers } = TestUtils.#getVersion(
  ...
  isVersionGreaterThan(minimumVersion: Array<number> | undefined): boolean {
    ...
    const lastIndex =
      Math.min(this.#VERSION_NUMBERS.length, minimumVersion.length) - 1;
