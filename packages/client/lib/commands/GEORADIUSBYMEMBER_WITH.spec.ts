...
describe("GEORADIUSBYMEMBER WITH", () => {
  ...
  testUtils.testWithClient(
    "client.geoRadiusByMemberWith",
    async (client) => {
      assert.deepEqual(
        await client.geoRadiusByMemberWith("key", "member", 3, "m", [
  ...
  testUtils.testWithCluster(
    "cluster.geoRadiusByMemberWith",
    async (cluster) => {
      assert.deepEqual(
        await cluster.geoRadiusByMemberWith("key", "member", 3, "m", [
          GeoReplyWith.DISTANCE,
        ]),
        []
      );
