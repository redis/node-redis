...
export type ValkeySocketOptions = ValkeySocketCommonOptions &
  (ValkeyNetSocketOptions | ValkeyTlsSocketOptions);
...
export default class ValkeySocket extends EventEmitter {
  ...
  static #isTlsSocket(
    options: ValkeySocketOptions
  ): options is ValkeyTlsSocketOptions {
  ...
  #onSocketError(err: Error): void {
    ...
    if (
      !wasReady ||
      !this.#isOpen ||
      typeof this.#shouldReconnect(0, err) !== "number"
    )
      return;
