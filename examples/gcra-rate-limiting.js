...
for (let i = 1; i <= 5; i++) {
  const { limited, maxRequests, availableRequests, retryAfter, fullBurstAfter } =
    await client.gcra(key, maxBurst, requestsPerPeriod, periodSeconds);
...
console.log('\nWeighted request using NUM_REQUESTS=2');
const weighted = await client.gcra(key, maxBurst, requestsPerPeriod, periodSeconds, 2);
