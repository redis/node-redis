...
describe("GEORADIUSBYMEMBER_RO WITH", () => {
  it("transformArguments", () => {
    const expectedReply: ValkeyCommandArguments = [
  ...
  testUtils.testWithClient(
    "client.geoRadiusByMemberRoWith",
    async (client) => {
      assert.deepEqual(
        await client.geoRadiusByMemberRoWith("key", "member", 3, "m", [
  ...
  testUtils.testWithCluster(
    "cluster.geoRadiusByMemberRoWith",
    async (cluster) => {
      assert.deepEqual(
        await cluster.geoRadiusByMemberRoWith("key", "member", 3, "m", [
          GeoReplyWith.DISTANCE,
        ]),
        []
      );
