...
describe('TokenManager', () => {
  ...
  /**
   * Helper function to create a test token with a given TTL and clock time.
   * @param ttlSeconds
   * @param clock
   */
  const createToken = (ttlSeconds: number, clock: FakeClock): Token<string> => {
    const now = clock.now();
    return new Token('test-token', now + ttlSeconds, now);
  };
  ...
  describe('calculateRefreshTime', () => {
    it('should calculate correct refresh time with 0.8 ratio', () => {
      ...
      const manager = new TokenManager(new TestIdentityProvider(), config, clock);
      const token = createToken(1000, clock);
      const refreshTime = manager.calculateRefreshTime(token);
    ...
    it('should handle short TTL tokens', () => {
      ...
      const manager = new TokenManager(new TestIdentityProvider(), config, clock);
      const token = createToken(5, clock);
      const refreshTime = manager.calculateRefreshTime(token);
    ...
    it('should handle expired tokens', () => {
      ...
      const token = createToken(-100, clock);
      const refreshTime = manager.calculateRefreshTime(token);
    ...
    describe('token refresh scenarios', () => {
      ...
      describe('token refresh', () => {
        it('should handle token refresh', async () => {
          ...
          const manager = new TokenManager(identityProvider, config, clock);
          ...
          assert.equal(listener.receivedTokens[0].expiresAtMs, 100, 'Should have correct expiration time');
          assert.equal(listener.receivedTokens[0].receivedAtMs, 0, 'Should have correct receivedAt time');
          ...
          assert.equal(listener.receivedTokens[1].expiresAtMs, 200, 'Should have correct expiration time');
          assert.equal(listener.receivedTokens[1].receivedAtMs, 100, 'Should have correct receivedAt time');
          assert.equal(listener.errors.length, 0, 'Should not have any errors');
          ...
          assert.equal(listener.receivedTokens[2].expiresAtMs, 300, 'Should have correct expiration time');
          assert.equal(listener.receivedTokens[2].receivedAtMs, 200, 'Should have correct receivedAt time');
          assert.equal(listener.errors.length, 0, 'Should not have any errors');
  ...
  describe('TokenManager error handling', () => {
    ...
    describe('error scenarios', () => {
      ...
      it('should handle retries with exponential backoff', async () => {
        ...
        // Simulate a scenario where the first token works, then we get two failures, then success
        const identityProvider = new ErrorSimulatingProvider(
          [
            'initial-token',
