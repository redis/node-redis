...
| keyPrefix                  |           | Prefix prepended to every key sent to Redis (ioredis-compatible). See [Key Prefixing](./client-configuration.md#key-prefixing).                                                                                                                                                                                       |
| commandOptions             |           | Default [Command Options](./command-options.md) (e.g. `timeout`, `typeMapping`) for commands executed through sentinel                                                                                                                                                                                                 |
| clientSideCache            |           | Client Side Caching configuration for sentinel (RESP3 only; shared across clients managed by sentinel). See [V5 docs](./v5.md#client-side-caching)                                                                                                                                                                     |
