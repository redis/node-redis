...
function RedisClient(stream, options) {
    ...
    this.max_attempts =
      (options.max_attempts && !isNaN(options.max_attempts) && options.max_attempts > 0 )
      ? options.max_attempts*1 : false; 
    this.command_queue = new Queue(); // holds sent commands to de-pipeline them
    this.offline_queue = new Queue(); // holds commands issued but not able to be sent
    this.commands_sent = 0;
    //default to infinite
    this.connect_timeout =
      (options.connect_timeout && !isNaN(options.connect_timeout) && options.connect_timeout > 0 )
      ? options.connect_timeout*1 : false;
