import { strict as assert } from 'assert';
import testUtils, { GLOBAL } from '../test-utils';
import { transformArguments } from './MREVRANGEWITHLABELS';
import { TimeSeriesAggregationType, TimeSeriesReduceType } from '.';

describe('MREVRANGEWITHLABELS', () => {
    describe('transformArguments', () => {
        it('without options', () => {
            assert.deepEqual(
                transformArguments(0, 10, ['name=value', 'age!=']),
                ['TS.MREVRANGE', '0', '10', 'WITHLABELS', 'FILTER', 'name=value', 'age!=']
            );
        });

        it('with SELECTED_LABELS', () => {
            assert.deepEqual(
                transformArguments(0, 10, ['name=value'], { 
                    SELECTED_LABELS: 'age' 
                }),
                ['TS.MREVRANGE', '0', '10', 'SELECTED_LABELS', 'age', 'FILTER', 'name=value']
            );
        });

        it('with FILTER_BY_TS, FILTER_BY_VALUE, COUNT, ALIGN, AGGREGATION, GROUPBY and REDUCE', () => {
            assert.deepEqual(
                transformArguments(0, 10, ['name=value'], {
                    FILTER_BY_TS: [1],
                    FILTER_BY_VALUE: {
                        min: 4,
                        max: 6
                    },
                    COUNT: 1,
                    ALIGN: '+',
                    AGGREGATION: { 
                        type: TimeSeriesAggregationType.FIRST, 
                        timeBucket: 5 
                    },
                    GROUPBY: 'age',
                    REDUCE: TimeSeriesReduceType.SUM,
                    SELECTED_LABELS: ['name', 'age']
                }),
                ['TS.MREVRANGE', '0', '10', 'FILTER_BY_TS', '1', 'FILTER_BY_VALUE', 
                '4', '6', 'COUNT', '1', 'ALIGN', '+', 'AGGREGATION', 'first', '5',
                'SELECTED_LABELS', 'name', 'age', 'FILTER', 'name=value', 'GROUPBY', 
                'age', 'REDUCE', 'sum']
            );
        });
    });

    testUtils.testWithClient('client.ts.mrangewithlabels', async client => {
        await Promise.all([
            client.ts.create('key1', {
                LABELS: { MREVRANGEkey: 'MREVRANGEvalue'}
            }),
            client.ts.add('key1', 1, 2)
        ]);

        assert.deepEqual(
            await client.ts.mRevRangeWithLabels(0, 100, ['MREVRANGEkey=MREVRANGEvalue'], {
                COUNT: 1
            }),
            [
                {
                    key: 'key1',
                    labels: { MREVRANGEkey: 'MREVRANGEvalue' },
                    samples: [
                        {
                            timestamp: 1,
                            value: 2
                        }
                    ]
                }
            ]
        );
    }, GLOBAL.SERVERS.OPEN);
});
