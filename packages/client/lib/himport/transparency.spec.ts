...
describe('HIMPORT transparency layer', () => {
  ...
  describe('cluster', () => {
    testUtils.testWithCluster('prepare fans out; SETs route by slot; discardAll is registry-based', async cluster => {
      // Pins the §7 response-policy fix: without the all_succeeded override the fan-out
      // reply reduction throws 'Unsupported reply type for merge aggregation'.
