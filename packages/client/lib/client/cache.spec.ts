  ...
  describe("CacheStats", () => {
    ...
    describe("instance methods", () => {
      ...
      describe("plus()", () => {
        ...
        it("should correctly sum large numbers saturating at Number.MAX_SAFE_INTEGER", () => {
          const statsA = CacheStats.of(Number.MAX_SAFE_INTEGER - 100, 1, 1, 1, 1, 1);
          const statsB = CacheStats.of(200, 1, 1, 1, 1, 1);
          let sum = statsA.plus(statsB);
          assert.strictEqual(sum.hitCount, Number.MAX_SAFE_INTEGER, "Summing to > MAX_SAFE_INTEGER should cap at MAX_SAFE_INTEGER");

          const statsC = CacheStats.of(Number.MAX_VALUE, 1, 1, 1, 1, 1);
          const statsD = CacheStats.of(Number.MAX_VALUE, 1, 1, 1, 1, 1);
          sum = statsC.plus(statsD);
          assert.strictEqual(sum.hitCount, Number.MAX_SAFE_INTEGER, "Summing MAX_VALUE should cap at MAX_SAFE_INTEGER");
        });
      });
