...
import { RedisArgument, ReplyUnion, TransformReply } from '../RESP/types';
...
export class BasicClientSideCache extends ClientSideCacheProvider {
  ...
  */
  override async handleCache(client: CachingClient, parser: CommandParser, fn: CmdFunc, transformReply: TransformReply | undefined) {
    let reply: ReplyUnion;

    const cacheKey = parser.redisArgs.join('_');
  ...
  override invalidate(key: RedisArgument | null) {
    ...
    if (set) {
     for (const cacheKey of set) {
        ...
        this.emit('cache-key-invalidated', cacheKey);
      }
  ...
  get(cacheKey?: string | undefined) {
    ...
    if (val && !val.validate()) {
      this.delete(cacheKey);
      this.emit(`cache-key-invalid`, cacheKey);
  ...
  set(cacheKey: string, cacheEntry: ClientSideCacheEntry, keys: ReadonlyArray<RedisArgument>) {
