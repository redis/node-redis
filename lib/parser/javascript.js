...
ParseResultError.prototype.__proto__ = Error.prototype;
...
ReplyParser.prototype._parseResult = function (type) {
    ...
    if (type === 43 || type === 45) { // + or -
        ...
        if (end > this._buffer.length) {
            this._offset = start;
            throw new ParseResultError("too far");
        }
...
ReplyParser.prototype.execute = function (buffer) {
    ...
    while (true) {
        ...
        try {
            ...
            if(! (err instanceof ParseResultError)) { //Errors from callbacks should bubble up
                throw err;
            }
