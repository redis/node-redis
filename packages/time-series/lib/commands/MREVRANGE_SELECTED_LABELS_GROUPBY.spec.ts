...
describe('TS.MREVRANGE_SELECTED_LABELS_GROUPBY', () => {
  ...
  testUtils.testWithClient('client.ts.mRevRangeSelectedLabelsGroupBy RESP3', async client => {
    const [, reply] = await Promise.all([
      client.ts.add('key', 0, 0, {
        LABELS: { label: 'value' }
      }),
    ...
    // RESP3 includes labels as a Map (converted to object) with selected labels
