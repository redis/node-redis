...
export type OnShardedChannelMoved = (
  channel: string,
  listeners: ChannelListeners
) => void;

export default class ValkeyCommandsQueue {
  static #flushQueue<T extends CommandWaitingForReply>(
    queue: LinkedList<T>,
    err: Error
  ): void {
    while (queue.length) {
  ...
  #decoder = new RESP2Decoder({
    ...
    onReply: (reply) => {
      if (this.#pubSub.isActive && Array.isArray(reply)) {
        ...
        const isShardedUnsubscribe = PubSub.isShardedUnsubscribe(
          reply as Array<Buffer>
        );
        if (isShardedUnsubscribe && !this.#waitingForReply.length) {
          ...
          isShardedUnsubscribe ||
          PubSub.isStatusReply(reply as Array<Buffer>)
        ) {
  ...
  addCommand<T = ValkeyCommandRawReply>(
    args: ValkeyCommandArguments,
  ...
  flushWaitingForReply(err: Error): void {
    ...
    while (
      this.#waitingToBeSent.head?.value.chainId === this.#chainInExecution
    ) {
