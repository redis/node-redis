...
describe("Cluster", () => {
  ...
  testUtils.testWithCluster(
    "scripts",
    async (cluster) => {
      assert.equal(await cluster.square(2), 4);
    },
    {
      ...GLOBAL.CLUSTERS.OPEN,
  ...
  it("should throw RootNodesUnavailableError", async () => {
    ...
    try {
      await assert.rejects(cluster.connect(), RootNodesUnavailableError);
    } catch (err) {
      await cluster.disconnect();
      throw err;
    }
  ...
  testUtils.testWithCluster(
    ...
    async (cluster) => {
      ...
      await Promise.all([
        importingClient.clusterSetSlot(
          slot,
          ClusterSlotStates.IMPORTING,
          migrating.id
        ),
        migratingClient.clusterSetSlot(
          slot,
          ClusterSlotStates.MIGRATING,
          importing.id
        ),
      ]);
      ...
      await migratingClient.migrate(importing.host, importing.port, key, 0, 10);
  ...
  testUtils.testWithCluster(
    "getRandomNode should spread the the load evenly",
    async (cluster) => {
  ...
  testUtils.testWithCluster(
    "getSlotRandomNode should spread the the load evenly",
    async (cluster) => {
  ...
  testUtils.testWithCluster(
    ...
    async (cluster) => {
      ...
      const { numberOfMasters, numberOfReplicas } =
        GLOBAL.CLUSTERS.WITH_REPLICAS;
      ...
      assert.equal(
        cluster.nodeByAddress.size,
        numberOfMasters + numberOfMasters * numberOfReplicas
      );
    },
    GLOBAL.CLUSTERS.WITH_REPLICAS
  );
  ...
  testUtils.testWithCluster(
    "getMasters should be backwards competiable (without `minimizeConnections`)",
    async (cluster) => {
  ...
  testUtils.testWithCluster(
    "getSlotMaster should be backwards competiable (without `minimizeConnections`)",
    async (cluster) => {
  ...
  testUtils.testWithCluster(
    "should send commands with commandOptions to correct cluster slot (without redirections)",
    async (cluster) => {
  ...
  describe("PubSub", () => {
    ...
    testUtils.testWithCluster(
      "concurrent UNSUBSCRIBE does not throw an error (#2685)",
      async (cluster) => {
        const listener = spy();
        await Promise.all([
    ...
    testUtils.testWithCluster(
      "should move listeners when PubSub node disconnects from the cluster",
      async (cluster) => {
        const listener = spy();
        await cluster.subscribe("channel", listener);

        assert.ok(cluster.pubSubNode);
        const [migrating, importing] =
            cluster.masters[0].address === cluster.pubSubNode.address
              ? cluster.masters
        ...
        const range =
          cluster.slots[0].master === migrating
            ? {
                key: "bar", // 5061
                start: 0,
                end: 8191,
    ...
    testUtils.testWithCluster(
      "concurrent SUNSUBCRIBE does not throw an error (#2685)",
      async (cluster) => {
        const listener = spy();
        await Promise.all([
    ...
    testUtils.testWithCluster(
      "should handle sharded-channel-moved events",
      async (cluster) => {
        const SLOT = 10328,
          migrating = cluster.slots[SLOT].master,
