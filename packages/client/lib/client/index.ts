...
import ValkeySocket, {
  ValkeySocketOptions,
  ValkeyTlsSocketOptions,
} from "./socket";
import ValkeyCommandsQueue, { QueueCommandOptions } from "./commands-queue";
import ValkeyClientMultiCommand, {
  ValkeyClientMultiCommandType,
} from "./multi-command";
...
> extends ValkeyExtensions<M, F, S> {
  ...
  name?: string;
  /**
   * Valkey database number (see [`SELECT`](https://redis.io/commands/select) command)
   */
  database?: number;
  /**
...
export type WithModules<M extends ValkeyModules> = {
  [P in keyof M as ExcludeMappedString<P>]: {
    [C in keyof M[P] as ExcludeMappedString<C>]: ValkeyCommandSignature<
      M[P][C]
    >;
  };
};
...
export type WithFunctions<F extends ValkeyFunctions> = {
  [P in keyof F as ExcludeMappedString<P>]: {
    [FF in keyof F[P] as ExcludeMappedString<FF>]: ValkeyCommandSignature<
      F[P][FF]
    >;
  };
};
...
type ClientLegacyCallback = (
  err: Error | null,
  reply?: ValkeyCommandRawReply
) => void;
...
> extends EventEmitter {
  static commandOptions<T extends ClientCommandOptions>(
    options: T
  ): CommandOptions<T> {
  ...
  ): ValkeyClientOptions<M, F, S> | undefined {
    if (options?.url) {
  ...
  #initiateSocket(): ValkeySocket {
    const socketInitiator = async (): Promise<void> => {
      ...
      if (!this.#options?.disableClientInfo) {
        promises.push(
          this.#queue
            .addCommand(["CLIENT", "SETINFO", "LIB-VER", version], {
              asap: true,
            })
            .catch((err) => {
  ...
  duplicate(
    overrides?: Partial<ValkeyClientOptions<M, F, S>>
  ): ValkeyClientType<M, F, S> {
  ...
  executeFunction(
    ...
    return this.#sendCommand(fCallArguments(name, fn, args), options);
  }
  ...
  SUBSCRIBE<T extends boolean = false>(
    ...
    return this.#pubSubCommand(
      this.#queue.subscribe(PubSubType.CHANNELS, channels, listener, bufferMode)
    );
  }
  ...
  PSUBSCRIBE<T extends boolean = false>(
    ...
    return this.#pubSubCommand(
      this.#queue.subscribe(PubSubType.PATTERNS, patterns, listener, bufferMode)
    );
  }
  ...
  SSUBSCRIBE<T extends boolean = false>(
    ...
    return this.#pubSubCommand(
      this.#queue.subscribe(PubSubType.SHARDED, channels, listener, bufferMode)
    );
  }
  ...
  executeIsolated<T>(
    fn: (client: ValkeyClientType<M, F, S>) => T | Promise<T>
  ): Promise<T> {
