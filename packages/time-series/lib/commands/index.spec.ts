...
describe("transformTimestampArgument", () => {
  it("number", () => {
    assert.equal(transformTimestampArgument(0), "0");
  });

  it("Date", () => {
    assert.equal(transformTimestampArgument(new Date(0)), "0");
  });

  it("string", () => {
    assert.equal(transformTimestampArgument("*"), "*");
  });
});

function testOptionalArgument(
  fn: (args: ValkeyCommandArguments) => unknown
): void {
  it("undefined", () => {
    assert.deepEqual(fn([]), []);
  });
...
describe("pushRetentionArgument", () => {
  ...
  it("number", () => {
    assert.deepEqual(pushRetentionArgument([], 1), ["RETENTION", "1"]);
  });
});
...
describe("pushChunkSizeArgument", () => {
  ...
  it("number", () => {
    assert.deepEqual(pushChunkSizeArgument([], 1), ["CHUNK_SIZE", "1"]);
  });
});
...
describe("pushLabelsArgument", () => {
  ...
  it("{ label: 'value' }", () => {
    assert.deepEqual(pushLabelsArgument([], { label: "value" }), [
      "LABELS",
      "label",
      "value",
    ]);
...
describe("transformIncrDecrArguments", () => {
  it("without options", () => {
    assert.deepEqual(transformIncrDecrArguments("TS.INCRBY", "key", 1), [
      "TS.INCRBY",
      "key",
      "1",
    ]);
...
it("transformSampleReply", () => {
  assert.deepEqual(transformSampleReply([1, "1.1"]), {
    timestamp: 1,
    value: 1.1,
  });
});
...
describe("pushRangeArguments", () => {
  it("without options", () => {
    assert.deepEqual(pushRangeArguments([], "-", "+"), ["-", "+"]);
  });
...
describe("pushMRangeGroupByArguments", () => {
  it("undefined", () => {
    assert.deepEqual(pushMRangeGroupByArguments([]), []);
  });
...
describe("pushFilterArgument", () => {
  it("string", () => {
    assert.deepEqual(pushFilterArgument([], "label=value"), [
      "FILTER",
      "label=value",
    ]);
  });
  ...
  it("Array", () => {
    assert.deepEqual(pushFilterArgument([], ["1=1", "2=2"]), [
      "FILTER",
      "1=1",
      "2=2",
    ]);
...
describe("pushMRangeArguments", () => {
  it("without options", () => {
    assert.deepEqual(pushMRangeArguments([], "-", "+", "label=value"), [
      "-",
      "+",
      "FILTER",
      "label=value",
...
describe("pushWithLabelsArgument", () => {
  it("without selected labels", () => {
    assert.deepEqual(pushWithLabelsArgument([]), ["WITHLABELS"]);
  });

  it("with selected labels", () => {
    assert.deepEqual(pushWithLabelsArgument([], ["label"]), [
      "SELECTED_LABELS",
      "label",
    ]);
  });
...
it("pushMRangeWithLabelsArguments", () => {
  assert.deepEqual(pushMRangeWithLabelsArguments([], "-", "+", "label=value"), [
    "-",
    "+",
    "WITHLABELS",
    "FILTER",
...
describe("pushLatestArgument", () => {
  it("undefined", () => {
    assert.deepEqual(pushLatestArgument([]), []);
  });

  it("false", () => {
    assert.deepEqual(pushLatestArgument([], false), []);
  });

  it("true", () => {
    assert.deepEqual(pushLatestArgument([], true), ["LATEST"]);
  });
});
