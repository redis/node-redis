...
export interface Fieldset {
   ...
   * sorted, or reordered (the server pairs SET values to this order positionally). Holds a
   * sliced copy, never an alias of the caller's array; mutating a Buffer's *contents* in
   * place still aliases — conventional Node.js behavior.
   */
 ...
 * DISCARD/DISCARDALL replies. Mutated only by the per-connection command hook — safe because
 * `set` is idempotent (cluster fan-out runs the hook once per master against this same
 * instance) and `discard`/`discardAll` are effective-only.
 */
export class FieldsetRegistry {
  /**
   * Fieldset name → registration. Map keys are `String()`-coerced names (a Buffer key would
   * be identity-compared by `Map` and poison lookups); values hold sliced copies, never
   * aliases of caller arrays.
   */
  #fieldsets = new Map<string, Fieldset>();

  /**
   * Source of `Fieldset.version`. Registry-wide and never reset — a per-entry counter
   * restarting at 0 after discard + re-prepare would make a stale connection entry compare
   * as fresh, skip the re-PREPARE, and silently bind values against the old field list.
   */
  #versionCounter = 0;

  /**
   * Counts EFFECTIVE discards only (a discard of an unknown name does not bump), so no-op
   * discards never force fleet-wide reconciles and a cluster fan-out of one user DISCARD
   * bumps exactly once (the first hook run deletes, the rest early-return). Connections
  ...
  /**
   * Names a connection's session still holds but the registry no longer does — i.e. the
   * DISCARDs that connection must replay to catch up. Names that were discarded and
   * re-registered are NOT returned: the version-gated lazy PREPARE (a silent server-side
   * replace) covers them without a DISCARD.
   */
  pendingDiscards(sessionNames: Set<string>): Set<string> {
